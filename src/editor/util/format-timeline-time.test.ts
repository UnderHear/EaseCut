...
describe('formatTimelineTime', () => {
  it('formats minutes, seconds, and centiseconds', () => {
    ...
    expect(formatTimelineTime(9)).toBe('00:09:00');
    expect(formatTimelineTime(9.12)).toBe('00:09:12');
    expect(formatTimelineTime(62.03)).toBe('01:02:03');
    expect(formatTimelineTime(6_001.99)).toBe('100:01:99');
  });

  it('truncates centiseconds without carrying into the next second', () => {
    expect(formatTimelineTime(9.999)).toBe('00:09:99');
    expect(formatTimelineTime(10)).toBe('00:10:00');
  });

  it('treats negative and non-finite values as zero', () => {
    expect(formatTimelineTime(-0.01)).toBe('00:00:00');
